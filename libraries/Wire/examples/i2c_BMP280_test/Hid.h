...
// ── Binary プロトコル定数 ─────────────────────────────────────────────────
#define HID_BINARY_MARKER 0x52
#define HID_BINARY_MORE 0x80
#define HID_BINARY_END 0x01
...
public:
  ...
  // ── バイナリ送信（EP3 → ブラウザ）──────────────────────────────
  // 既存 Print プロトコルとは別の 0x52 marker を使う。
  // 1 report あたり最大5byte。有効長 len を持つので 0x00 も送信可能。
...
private:
  ...
  void _binSend(uint8_t flags, const uint8_t* data, uint8_t n);
};
